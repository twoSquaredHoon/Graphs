...
function draw() {
  ...
  nodes.forEach(n => {
    ...
    ctx.globalAlpha = isDimmed ? 0.15 : isInline ? 0.7 : n.isGhost ? 0.4 : 0.88;
    ...
    if (isInline) {
      ...
      ctx.strokeStyle = 'rgba(124,111,205,0.9)'; ctx.lineWidth = 1.5 / zoom; ctx.stroke();
      ctx.setLineDash([]);
    } else {
      ctx.beginPath(); ctx.arc(n.x, n.y, r, 0, Math.PI * 2);
